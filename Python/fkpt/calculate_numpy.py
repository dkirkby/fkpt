...
# Module-level calculator instance for backward compatibility
_default_calculator = NumpyCalculator()


def init_cubic_spline(x: Float64NDArray, y: Float64NDArray) -> Float64NDArray:
    """Initialize a cubic spline interpolator by precomputing 2nd derivatives.

    Backward-compatible wrapper for the class method.
    """
    return _default_calculator._init_cubic_spline(x, y)
...
def eval_cubic_spline(xa: Float64NDArray, ya: Float64NDArray, y2a: Float64NDArray, x: Float64NDArray) -> Float64NDArray:
    ...
    Backward-compatible wrapper for the class method.
    """
    return _default_calculator._eval_cubic_spline(xa, ya, y2a, x)


def calculate(
        kfuncs_in,  # KFunctionsIn type
        A: float, ApOverf0: float, CFD3: float, CFD3p: float, sigma2v: float
    ):
    """Calculate k-functions using the old KFunctionsIn interface.

    Backward-compatible wrapper that converts old interface to new class-based interface.

    Args:
        kfuncs_in: KFunctionsIn namedtuple with k_in, logk_grid, kk_grid, Y, xxQ, wwQ, xxR, wwR
        A, ApOverf0, CFD3, CFD3p, sigma2v: Cosmological parameters

    Returns:
        KFunctionsOut containing all computed k-functions
    """
    from fkpt.types import KFunctionsInitData

    # Extract Y components (Pk, Pk_nw, fk)
    Pk_in, Pk_nw_in, fk_in = kfuncs_in.Y

    # Initialize calculator with grid data
    init_data = KFunctionsInitData(
        k_in=kfuncs_in.k_in,
        logk_grid=kfuncs_in.logk_grid,
        kk_grid=kfuncs_in.kk_grid,
        xxQ=kfuncs_in.xxQ,
        wwQ=kfuncs_in.wwQ,
        xxR=kfuncs_in.xxR,
        wwR=kfuncs_in.wwR,
    )
    _default_calculator.initialize(init_data)

    # Call evaluate with separated power spectra
    # Note: f0 is set to 1.0 for backward compatibility (fk_in is assumed already normalized)
    return _default_calculator.evaluate(
        Pk_in, Pk_nw_in, fk_in,
        A, ApOverf0, CFD3, CFD3p, sigma2v, f0=1.0
    )
